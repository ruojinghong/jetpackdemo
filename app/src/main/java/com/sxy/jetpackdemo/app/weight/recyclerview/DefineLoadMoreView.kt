 ...
 * @description:
 */
class DefineLoadMoreView(context : Context) :
    LinearLayout(context),SwipeRecyclerView.LoadMoreView,
    View.OnClickListener{
    val mProgressBar : ProgressBar
    private val mTvMessage : AppCompatTextView

    private var mLoadMoreListener : SwipeRecyclerView.LoadMoreListener? = null

    fun setLoadMoreListener(laodMoreListener: SwipeRecyclerView.LoadMoreListener){
        mLoadMoreListener = laodMoreListener
    }
    init {
        layoutParams = ViewGroup.LayoutParams(LayoutParams.MATCH_PARENT,LayoutParams.WRAP_CONTENT)
        gravity = Gravity.CENTER
        visibility = View.GONE
        minimumHeight = ConvertUtils.dp2px(36f)
